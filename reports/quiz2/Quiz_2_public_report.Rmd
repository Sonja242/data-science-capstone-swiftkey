---
title: "Next-Word Prediction: Quiz 2 Case Study"
...
**Sentence context.** The model reads text in small units called tokens, which may be whole words or parts of words. The existing limit of **128 tokens** was retained for consistency with the established predictor. A token may be a whole word or part of a word. For a word composed of multiple tokens, its score combines those parts and accounts for whether the word ends at that point. Each option is therefore assessed as a complete next word.
...
The original model agreed with **9/10 reviewed choices (90%)**. One choice changed during review. The final reviewed submission received **100%**. Agreement describes the review workflow; it is not an independently graded model-only attempt.
...
The **100% final grade** supports the success of the reviewed workflow on these ten cases. The saved model agreed with **`r model_matches`/10 final choices**, and **`r summary_data$review_changes`** changed during review. The revised choice was already among the model's leading alternatives. This motivates testing how ranking uses whole-sentence meaning and natural expressions. One successful change cannot establish the benefit of a general review rule or a new model. Sensitivity to another model remains untested in this application.
...
This report was **knitted from `Quiz_2_public_report.Rmd`** using the shared Milestone stylesheet. The **Code** menu reveals calculations. The published R Markdown, HTML and `public_summary.json` reproduce the tables and plots without private assessment files, model training or a GPU. The summary contains only aggregate counts checked against the local records.
...
[Public source and aggregate data](https://github.com/Sonja242/data-science-capstone-swiftkey/tree/main/reports/quiz2)
