# A fresh final holdout for the learning-curve experiment.
# Author: Sonja Sahebzad.
library(data.table)
source("R/predictive_model_v3.R")
setDTthreads(4L)
destination <- "data/learning_curve/final_test.csv"
...
chosen <- pick(eligible[split=="test" & !row_id %in% c(old_test,neural_test,adaptation_test)],
               300L,20261402L)
stopifnot(nrow(chosen)==900L, all(chosen$split=="test"),
          !any(chosen$row_id %in% c(old_test,neural_test,adaptation_test)))
...
jsonlite::write_json(list(
  ...
  cases=900L,cases_per_source=300L,selection_seed=20261402L,
  position_seed=20261403L,choice_seed=20261404L,
  excluded_prior_v3_test_lines=length(old_test),
  excluded_prior_neural_test_lines=length(neural_test),
  excluded_prior_adaptation_test_lines=length(adaptation_test),
  ...
  "models/learning_curve_holdout_manifest.json",auto_unbox=TRUE,pretty=TRUE)
fwrite(data.table(source=chosen$source,line_hash=case_hashes),
       "models/learning_curve_reserved_case_hashes.csv")
...
options <- make_choice_cases(cases,frequency,20261404L)
...
manifest <- jsonlite::read_json("models/learning_curve_holdout_manifest.json",simplifyVector=TRUE)
...
jsonlite::write_json(manifest,"models/learning_curve_holdout_manifest.json",auto_unbox=TRUE,pretty=TRUE)
cat("Reserved 900 fresh final cases; no model score has been calculated.\n")
